...
{
  numChiro = "Chiropracteur";
}

Chiropracteur::~Chiropracteur(){}
string Chiropracteur::getNum(){}
string Chiropracteur::getChuong(){}
void Chiropracteur::jeSuis()
{
  map<string, string>::iterator it;
  if (ani_malade.size() == 0) cout << "Animal da duoc chua het\n";
  else
    {
      cout << numChiro << " cham soc cac Animal nay: \n";
      for (it=ani_malade.begin(); it!=ani_malade.end(); ++it)
	cout <<"\t\tAnimal: " << it->first << "  benh:  " << it->second <<endl;
    }
}

void Chiropracteur::receive_malade(string maAni, string mal)
{
  ani_malade.insert( pair<string, string>(maAni,mal) );
}

void Chiropracteur::soigner_malade(list<Animal> &ani)
{
  string maAni;
